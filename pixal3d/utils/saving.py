...
from pixal3d.utils.typing import *
...
class SaverMixin:
    ...
    def create_loggers(self, cfg_loggers: DictConfig, full_config: Optional[DictConfig] = None) -> None:
        if "wandb" in cfg_loggers.keys() and cfg_loggers.wandb.enable:
            from omegaconf import OmegaConf
            # 将完整配置转换为字典，上传到 wandb 云端显示
            config_dict = OmegaConf.to_container(full_config, resolve=True) if full_config else None
            self._wandb_logger = WandbLogger(
                project=cfg_loggers.wandb.project, 
                name=cfg_loggers.wandb.name,
                config=config_dict  # 上传配置到 wandb
            )
