...
from ..attention import MultiHeadAttention,ProjectAttention
...
class ModulatedTransformerCrossBlock(nn.Module):
    ...
    def __init__(
        ...
        qkv_bias: bool = True,
        share_mod: bool = False,
        image_attn_mode:Literal["cross", "proj"] = "cross",
    ):
        super().__init__()
        self.use_checkpoint = use_checkpoint
        self.share_mod = share_mod
        ...
        if image_attn_mode == "cross":
            ...
            self.cross_attn = ProjectAttention(cross_attn)
        else:
    ...
    def _forward(self, x: torch.Tensor, mod: torch.Tensor, context: torch.Tensor):
        ...
        h = self.norm2(x)
        h = self.cross_attn(h, context) # h = h + context
        x = x + h
